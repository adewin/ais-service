...
public class FileUploadServiceSupplier implements Supplier<FileUploadService> {
  ...
  private static S3ObjectExtractor s3ObjectExtractorInstance = null;
  private static S3ObjectCopier s3ObjectCopierInstance = null;

  @Override
  public FileUploadService get() {
    return new FileUploadService(getS3ObjectExtractorInstance(), getS3ObjectCopierInstance());
  }
  ...
  private static S3ObjectExtractor getS3ObjectExtractorInstance() {
    if (s3ObjectExtractorInstance == null) {
      s3ObjectExtractorInstance = new S3ObjectExtractor();
    }
    return s3ObjectExtractorInstance;
  }
}
